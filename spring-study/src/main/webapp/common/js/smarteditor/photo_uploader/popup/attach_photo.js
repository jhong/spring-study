	...
	function checkDragAndDropAPI(){
		/*
		 * 항상 일반 업로드 사용하도록 주석처리 by jhong (2014.05.15)
		try{
			if( !oNavigator.ie ){
    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/spring-study/attach.do?command=registSmart'; 	//upload URL
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			...
 			sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/attach.do?command=registSmart',	//샘플 URL입니다.
